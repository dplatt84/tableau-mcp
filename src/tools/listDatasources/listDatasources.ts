...
export const getListDatasourcesTool = (server: Server): Tool<typeof paramsSchema> => {
  const listDatasourcesTool = new Tool({
    ...
    description: `
    Retrieves published data sources from a Tableau site. Supports filtering via field:operator:value expressions. Use to list search or filter data sources. FILTER FIELDS: authenticationType connectionTo connectionType contentUrl createdAt databaseName description favoritesTotal hasAlert hasEmbeddedPassword hasExtracts isCertified isConnectable isPublished name ownerDomain ownerEmail ownerName projectName serverName size tableName tags type updatedAt. OPERATORS: eq equals gt greater than gte greater equal in list lt less than lte less equal. SYNTAX: Format field:operator:value multiple filters comma-separated case-sensitive. Date format ISO 8601 like 2023-01-01T00:00:00Z. Wildcards use star for matching. Values cannot contain special chars or commas. EXAMPLES: Empty filter lists all. name:eq:Project Views filters by name. projectName:eq:Finance filters by project. Combined name:eq:Views,projectName:eq:Finance,createdAt:gt:2023-01-01T00:00:00Z.
    `,
    paramsSchema,
    annotations: {
